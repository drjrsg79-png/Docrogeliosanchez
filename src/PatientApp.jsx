        ...
        {view==="plan"&&(
          <>
            ...
            {dietPlan&&<>
              ...
              {(dietPlan.dias||[]).map((d,i)=><Card key={i}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:15}}>{d.dia}</div><Badge color={C.navy}>{d.kcal} kcal</Badge></div>{[["Desayuno",d.desayuno],["Almuerzo",d.almuerzo],["Cena",d.cena],["Snack",d.snack]].map(([t,c])=>c&&<div key={t} style={{marginBottom:8,paddingLeft:10,borderLeft:`3px solid ${C.deep}`}}><div style={{fontSize:10,color:C.muted,fontWeight:700,textTransform:"uppercase"}}>{t}</div><div style={{fontSize:13,color:C.inkSec}}>{typeof c==="string"?c:(c.descripcion||"")}{typeof c==="object"&&c.calorias?` (${c.calorias} kcal)`:""}</div></div>)}</Card>)}
              <PBtn onClick={()=>{setDietPlan(null);genDiet();}} color={C.deep} textColor={C.navy}>Regenerar</PBtn>
            </>}
          </>
        )}
