...
function Exercise({profile,exerciseLog,setExerciseLog,workoutPlan,setWorkoutPlan}) {
  ...
  async function genWorkout(){
    setWLoad(true);
    const text=await callClaude(`Entrenador personal certificado. SOLO JSON válido sin texto extra.`,
      `Plan semanal. ${profile.sex}, ${profile.age}a, ${profile.weight}kg, objetivo:${profile.goal}, nivel:${profile.activity}.\nJSON:{"objetivo":"...","semanas":8,"split":[{"dia":"Lunes","nombre":"...","tipo":"Fuerza","duracion":"X min","ejercicios":[{"nombre":"nombre del ejercicio","series":"3x12","descanso":"60s","nota":"clave técnica"}],"cardio":"opcional"}],"principios":["p1","p2","p3"],"progresion":"..."}`,1500);
    try{setWorkoutPlan(JSON.parse(text.replace(/```json|```/g,"").trim()));}catch{setWorkoutPlan(null);}
    setWLoad(false);
  }
  ...
  return (
    <div style={{paddingBottom:110}}>
      ...
      <div style={{padding:"16px 16px 0"}}>
        ...
        {view==="log"&&<>
          <PBtn onClick={()=>setModal(true)} style={{marginBottom:16}}>+ Registrar ejercicio</PBtn>
          {exerciseLog.length===0?<Card style={{textAlign:"center",padding:28}}><div style={{fontSize:36,marginBottom:10}}>🏃</div><div style={{color:C.muted,fontSize:14}}>Registra tu primer ejercicio del día</div></Card>
          :exerciseLog.map((e,i)=><Card key={i}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:46,height:46,borderRadius:12,background:C.navy+"18",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:20}}>🏋️</div><div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{e.name}</div><div style={{color:C.muted,fontSize:12}}>{e.mins} min · {e.cat}</div></div><div style={{textAlign:"center"}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:20,color:C.scarlet,fontWeight:800}}>{e.burned}</div><div style={{fontSize:9,color:C.scarlet}}>kcal</div></div></div></Card>)}
        </>}

        {view==="plan"&&<>
          {!workoutPlan&&!wLoad&&<Card style={{textAlign:"center",padding:28}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:18,color:C.ink,marginBottom:8}}>Tu rutina personalizada</div><div style={{color:C.muted,fontSize:13,marginBottom:18}}>Plan de 8 semanas basado en tu objetivo y nivel de actividad.</div><PBtn onClick={genWorkout}>Generar mi rutina</PBtn></Card>}
          {wLoad&&<Card style={{textAlign:"center",padding:36}}><Spin/><div style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:C.navy}}>Diseñando tu rutina...</div></Card>}
          {workoutPlan&&<>
            <Card style={{background:C.navy}}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:"#FFF",marginBottom:6}}>{workoutPlan.semanas} semanas · {workoutPlan.split?.length} días/semana</div>
              <div style={{color:"rgba(255,255,255,0.6)",fontSize:13,marginBottom:10}}>{workoutPlan.objetivo}</div>
              {(workoutPlan.principios||[]).map((p,i)=><div key={i} style={{fontSize:12,color:"rgba(255,255,255,0.65)",marginBottom:5,paddingLeft:10,borderLeft:`2px solid ${C.gold}`}}>{p}</div>)}
            </Card>
            {(workoutPlan.split||[]).map((d,i)=><Card key={i}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                <div><div style={{fontFamily:"'Playfair Display',serif",fontSize:15,color:C.ink}}>{d.dia}</div><div style={{fontSize:12,color:C.muted}}>{d.nombre}</div></div>
                <Badge color={C.cobalt}>{d.duracion}</Badge>
              </div>
              {(d.ejercicios||[]).map((ex,j)=><div key={j} style={{padding:"8px 0",borderTop:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontWeight:600,fontSize:13}}>{ex.nombre}</span>
                  <div style={{display:"flex",gap:6,alignItems:"center"}}>
                    <span style={{background:C.goldPale,color:C.gold,borderRadius:6,padding:"2px 8px",fontSize:11,fontWeight:700}}>{ex.series}</span>
                    <button onClick={()=>ytSearch(ex.nombre)} style={{width:26,height:26,borderRadius:"50%",background:"#FF0000",border:"none",display:"flex",alignItems:"center",justifyContent:"center"}}>
        ...
        {view==="videos"&&<>
          <SI placeholder="Buscar ejercicio para ver tutorial..." value={search} onChange={e=>setSearch(e.target.value)} style={{marginBottom:14}}/>
          <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:10,marginBottom:14}}>
            {EX_CATS.map(c=><button key={c} onClick={()=>setActiveCat(c)} style={{background:activeCat===c?"#FF0000":C.card,color:activeCat===c?"#FFF":C.muted,border:`1px solid ${activeCat===c?"#FF0000":C.border}`,borderRadius:20,padding:"7px 14px",fontSize:12,fontWeight:600,whiteSpace:"nowrap"}}>{c}</button>)}
          </div>
          {(search?filtered:EXERCISES.filter(e=>activeCat==="Todos"||e.cat===activeCat)).map((ex,i)=>(
            <button key={i} onClick={()=>ytSearch(ex.name)} style={{width:"100%",background:C.card,border:`1px solid ${C.border}`,borderRadius:16,marginBottom:10,overflow:"hidden",boxShadow:C.shadow,display:"block",padding:0,textAlign:"left"}}>
              <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px"}}>
                <div style={{width:44,height:44,borderRadius:12,background:"#FF000020",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <div style={{width:0,height:0,borderTop:"9px solid transparent",borderBottom:"9px solid transparent",borderLeft:"16px solid #FF0000",marginLeft:4}}/>
                </div>
                <div style={{flex:1}}><div style={{fontWeight:700,fontSize:14,color:C.ink}}>{ex.name}</div><div style={{fontSize:11,color:C.muted}}>{ex.cat} · Ver en YouTube</div></div>
                <Badge color={C.navy}>MET {ex.met}</Badge>
              </div>
            </button>
          ))}
        </>}
      ...
      {modal&&<div style={{position:"fixed",inset:0,background:"rgba(21,46,68,0.55)",zIndex:300,display:"flex",flexDirection:"column",justifyContent:"flex-end",backdropFilter:"blur(4px)"}}>
        <div style={{background:C.card,borderRadius:"24px 24px 0 0",maxHeight:"92vh",display:"flex",flexDirection:"column",boxShadow:"0 -8px 48px rgba(21,46,68,0.2)"}}>
          <div style={{padding:"20px 20px 0",flexShrink:0}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:19,color:C.ink}}>Registrar ejercicio</div>
          ...
          <div style={{overflowY:"auto",flex:1,padding:"4px 20px 8px"}}>
            {filtered.map(e=><button key={e.name} onClick={()=>setSelected(e)} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"10px 0",background:"none",border:"none",borderBottom:`1px solid ${C.border}`,textAlign:"left"}}>
              <div style={{width:40,height:40,background:C.navy+"15",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:18}}>🏋️</div>
              <div style={{flex:1}}><div style={{fontWeight:600,fontSize:13,color:selected?.name===e.name?C.navy:C.ink}}>{e.name}</div><div style={{fontSize:11,color:C.muted}}>{e.cat} · MET {e.met}</div></div>
              {selected?.name===e.name&&<div style={{width:22,height:22,background:C.navy,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",color:"#FFF",fontSize:13}}>✓</div>}
            </button>)}
          </div>
          {selected&&<div style={{padding:"14px 20px 28px",borderTop:`1px solid ${C.border}`,flexShrink:0}}>
            <div style={{fontWeight:700,fontSize:14,marginBottom:10}}>{selected.name}</div>
            <FL>Duración (minutos)</FL>
            <SI type="number" value={mins} onChange={e=>setMins(e.target.value)} style={{marginBottom:10}}/>
            <div style={{display:"flex",alignItems:"center",gap:12,background:C.muted2,borderRadius:12,padding:"10px 16px",marginBottom:12}}>
              <div style={{flex:1,fontSize:13,color:C.muted}}>Calorías estimadas</div>
...
function Nutrition({profile,meals,setMeals}) {
  ...
  const [planLoad,setPlanLoad]=useState(false);
  const [view,setView]=useState("log");
  const SLOTS=[{k:"breakfast",l:"Desayuno",icon:"🌅"},{k:"lunch",l:"Almuerzo",icon:"☀️"},{k:"dinner",l:"Cena",icon:"🌙"},{k:"snacks",l:"Snacks",icon:"🍎"}];
  const totalCals=Object.values(meals).flat().reduce((s,f)=>s+f.cals,0);

  async function analyze(){
    if(!query.trim()) return;
    setLoading(true);
    const text=await callClaude(`Nutricionista experto. SOLO JSON sin texto extra: {"name":"nombre","cals":numero,"carbs":numero,"prot":numero,"fat":numero,"portion":"porción"}`,query);
    try{setResult(JSON.parse(text.replace(/```json|```/g,"").trim()));}catch{setResult({name:query,cals:0,carbs:0,prot:0,fat:0,portion:""});}
    setLoading(false);
  }
  ...
  return (
    <div style={{paddingBottom:110}}>
      ...
      <div style={{padding:"16px 16px 0"}}>
        <div style={{display:"flex",gap:8,marginBottom:18}}>
          <Chip onClick={()=>setView("log")} active={view==="log"}>Registro</Chip>
          <Chip onClick={()=>setView("plan")} active={view==="plan"}>Plan IA</Chip>
        </div>
        {view==="log"&&SLOTS.map(s=><Card key={s.k}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
            <span style={{fontSize:22,flexShrink:0}}>{s.icon}</span>
            <div style={{flex:1}}><div style={{fontWeight:700,fontSize:15}}>{s.l}</div><div style={{fontSize:12,color:C.muted}}>{meals[s.k].reduce((a,f)=>a+f.cals,0)} kcal</div></div>
            <button onClick={()=>{setSlot(s.k);setModal(true);}} style={{background:C.navy,color:"#FFF",border:"none",borderRadius:10,padding:"8px 14px",fontWeight:700,fontSize:13,boxShadow:C.shadow}}>+ Añadir</button>
          </div>
          {meals[s.k].map((f,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderTop:`1px solid ${C.border}`,fontSize:14}}><span style={{color:C.inkSec}}>{f.name} <span style={{color:C.ghost,fontSize:12}}>· {f.portion}</span></span><span style={{color:C.gold,fontWeight:700}}>{f.cals}</span></div>)}
        </Card>)}
        {view==="plan"&&<>
          {!dietPlan&&!planLoad&&<Card style={{textAlign:"center",padding:28}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:17,marginBottom:8}}>Tu plan de alimentación</div><div style={{color:C.muted,fontSize:13,marginBottom:18}}>Plan personalizado de 7 días basado en tu objetivo y metabolismo.</div><PBtn onClick={genDiet}>Generar mi plan</PBtn></Card>}
          {planLoad&&<Card style={{textAlign:"center",padding:36}}><Spin/><div style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:C.navy}}>Creando tu plan...</div></Card>}
          {dietPlan&&<>
            <Card style={{background:C.navy}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:"#FFF",marginBottom:4}}>{dietPlan.caloriasDiarias} kcal/día</div><div style={{color:"rgba(255,255,255,0.6)",fontSize:13}}>{dietPlan.objetivo}</div></Card>
            {(dietPlan.dias||[]).map((d,i)=><Card key={i}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:15}}>{d.dia}</div><Badge color={C.navy}>{d.kcal} kcal</Badge></div>{[["🌅 Desayuno",d.desayuno],["☀️ Almuerzo",d.almuerzo],["🌙 Cena",d.cena],["🍎 Snack",d.snack]].map(([t,c])=>c&&<div key={t} style={{marginBottom:8,paddingLeft:10,borderLeft:`3px solid ${C.deep}`}}><div style={{fontSize:10,color:C.muted,fontWeight:700,textTransform:"uppercase"}}>{t}</div><div style={{fontSize:13,color:C.inkSec}}>{c}</div></div>)}</Card>)}
            <PBtn onClick={()=>{setDietPlan(null);genDiet();}} color={C.deep} textColor={C.navy}>Regenerar</PBtn>
          </>}
        </>}
      </div>
      {modal&&<div style={{position:"fixed",inset:0,background:"rgba(21,46,68,0.55)",zIndex:300,display:"flex",flexDirection:"column",justifyContent:"flex-end",backdropFilter:"blur(4px)"}}>
        <div style={{background:C.card,borderRadius:"24px 24px 0 0",padding:24,maxHeight:"88vh",overflowY:"auto",boxShadow:"0 -8px 48px rgba(21,46,68,0.2)"}}>
          ...
          <FL>Comida</FL>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>{SLOTS.map(s=><Chip key={s.k} onClick={()=>setSlot(s.k)} active={slot===s.k}>{s.icon} {s.l}</Chip>)}</div>
          <FL>Describe el alimento o plato</FL>
          <textarea value={query} onChange={e=>setQuery(e.target.value)} placeholder="ej. Avena con plátano y nueces, 1 taza" style={{background:C.muted2,border:`1.5px solid ${C.border}`,borderRadius:12,padding:"13px 16px",color:C.ink,fontSize:14,width:"100%",height:80,resize:"none",outline:"none"}}/>
          <PBtn onClick={analyze} disabled={loading||!query.trim()} style={{marginTop:12}}>{loading?"Analizando...":"Analizar calorías con IA"}</PBtn>
          {result&&<div style={{marginTop:14,background:C.muted2,borderRadius:16,padding:16}}>
            <div style={{fontWeight:700,fontSize:15,color:C.ink,marginBottom:4}}>{result.name}</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:14}}>
              {[{l:"Cal",v:result.cals,c:C.navy},{l:"Carbs",v:result.carbs,c:C.cobalt},{l:"Prot",v:result.prot,c:C.gold},{l:"Grasa",v:result.fat,c:C.emerald}].map(m=><div key={m.l} style={{background:C.card,borderRadius:12,padding:"10px 6px",textAlign:"center",boxShadow:C.shadow}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:17,fontWeight:800,color:m.c}}>{m.v}</div><div style={{fontSize:9,color:C.muted,textTransform:"uppercase"}}>{m.l}</div></div>)}
            </div>
            <PBtn onClick={addFood}>Añadir</PBtn>
          </div>}
        </div>
      </div>}
    </div>
...
// ══════════════════════════════════════════════════════
function Chat({profile,workoutPlan,meals,exerciseLog}) {
  const [msgs,setMsgs]=useState([{role:"assistant",content:`¡Hola ${profile.name}! 👋 Soy tu coach de salud del Dr. Rogelio Sánchez con IA.\n\nConozco tu perfil completo y puedo ayudarte con ejercicios, nutrición, glucosa, motivación y más. ¿En qué te ayudo hoy?`}]);
  const [input,setInput]=useState("");
  const [loading,setLoading]=useState(false);
  const bottomRef=useRef(null);
  const tdee=calcTDEE(profile),target=goalCals(tdee,profile.goal);
  const totalCal=Object.values(meals).flat().reduce((s,f)=>s+(f.cals||0),0);
  const burnedToday=exerciseLog.reduce((s,e)=>s+e.burned,0);

  const systemPrompt=`Eres el coach de salud del consultorio del Dr. Rogelio Sánchez (internista, nutriólogo, especialista en diabetes). Atiendes al paciente ${profile.name}.

PERFIL: ${profile.sex==="male"?"Hombre":"Mujer"}, ${profile.age} años, ${profile.weight}kg, ${profile.height}cm
OBJETIVO: ${profile.goal==="lose"?"Bajar de peso":profile.goal==="gain"?"Ganar músculo":profile.goal==="recomp"?"Recomposición":"Mantenimiento"}
ACTIVIDAD: ${profile.activity} | TDEE: ${tdee} kcal | Meta: ${target} kcal
HOYKAL: Consumido ${totalCal} kcal, Quemado ${burnedToday} kcal
${workoutPlan?`PLAN ACTIVO: ${workoutPlan.objetivo} (${workoutPlan.semanas} semanas)`:""}

INSTRUCCIONES:
- Responde en español, conciso, empático y motivador
- Máximo 3-4 párrafos cortos por respuesta
- Para temas médicos serios, siempre referir al Dr. Rogelio
- Personaliza siempre tu respuesta al perfil del paciente`;

  const QUICK=[
    {icon:"💪",text:"¿Cómo mejoro mi técnica en la sentadilla?"},
    {icon:"🥗",text:"¿Qué debo comer antes de entrenar?"},
    {icon:"🩸",text:"¿Cómo controlo mejor mi glucosa con la dieta?"},
    {icon:"😴",text:"¿Cuánto afecta el sueño a mis resultados?"},
    {icon:"📈",text:"¿Cuánto tiempo para ver resultados reales?"},
    {icon:"💊",text:"¿Qué suplementos básicos recomiendas?"},
  ];
  ...
  async function send(text) {
    ...
    const newMsgs=[...msgs,{role:"user",content:userText}];
    setMsgs(newMsgs);setInput("");setLoading(true);
    const reply=await callClaude(systemPrompt,newMsgs.map(m=>({role:m.role,content:m.content})),700);
    setMsgs(m=>[...m,{role:"assistant",content:reply||"Lo siento, hubo un problema. Intenta de nuevo."}]);
    setLoading(false);
  }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",paddingBottom:80}}>
      <div style={{background:"linear-gradient(145deg,#0A1A2E,#152E44)",padding:"52px 20px 16px",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{width:48,height:48,borderRadius:"50%",background:C.gold,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,boxShadow:`0 4px 16px ${C.gold}66`}}>🤖</div>
          <div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,color:"#FFF",fontWeight:800}}>Coach Dr. Rogelio</div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 16px 8px"}}>
        ...
        {msgs.length===1&&!loading&&<div style={{marginTop:8}}>
          <div style={{fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:10}}>Preguntas rápidas</div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {QUICK.map((q,i)=><button key={i} onClick={()=>send(q.text)} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"11px 14px",textAlign:"left",fontSize:14,color:C.inkSec,display:"flex",gap:10,alignItems:"center",boxShadow:C.shadow}}>
              <span style={{fontSize:18,flexShrink:0}}>{q.icon}</span><span>{q.text}</span>
            </button>)}
          </div>
        </div>}
        <div ref={bottomRef}/>
      ...
      <div style={{position:"fixed",bottom:70,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:C.bg,borderTop:`1px solid ${C.border}`,padding:"12px 16px"}}>
        <div style={{display:"flex",gap:10,alignItems:"flex-end"}}>
          <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}}} placeholder="Escribe tu pregunta..." rows={1} style={{flex:1,background:C.card,border:`1.5px solid ${C.border}`,borderRadius:20,padding:"12px 16px",color:C.ink,fontSize:14,resize:"none",outline:"none",lineHeight:1.5,maxHeight:120}}/>
          <button onClick={()=>send()} disabled={!input.trim()||loading} style={{width:48,height:48,borderRadius:"50%",background:input.trim()&&!loading?C.navy:C.muted2,border:"none",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:input.trim()&&!loading?C.shadowMd:"none",transition:"all 0.2s"}}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={input.trim()&&!loading?"#FFF":C.muted} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
          </button>
        </div>
...
// ══════════════════════════════════════════════════════
const PEPTIDOS=[
  {name:"BPC-157",full:"Body Protection Compound",cat:"Recuperación",color:C.emerald,dose:"250–500 mcg/día",benefits:"Cicatrización, antiinflamatorio, protección gástrica.",recon:"2mg + 1-2ml agua bacteriostática → 1000-2000 mcg/ml\n• 500mcg = 50U · 250mcg = 25U",apply:"Subcutáneo 45°, abdomen o muslo.",cycle:"4–8 sem activo, descanso 2–4",timing:"Mañana en ayunas o antes de dormir"},
  {name:"TB-500",full:"Thymosin Beta-4",cat:"Recuperación",color:C.cobalt,dose:"2–2.5 mg, 2x/semana",benefits:"Regeneración muscular, flexibilidad articular, tendones.",recon:"5mg + 1ml agua bacteriostática → 5mg/ml\n• 2.5mg = 0.5ml = 50U",apply:"Subcutáneo o intramuscular.",cycle:"4–6 sem carga → mantenimiento",timing:"Días de entrenamiento"},
  {name:"Ipamorelin + CJC-1295",full:"GHRP + GHRH Stack",cat:"GH / Composición",color:C.navyMid,dose:"100–300 mcg c/u, 1–3x/día",benefits:"Liberación de GH, composición corporal, sueño profundo.",recon:"2mg + 2ml agua bact. → 1000mcg/ml\n• 200mcg = 0.2ml c/u (misma jeringa)",apply:"Subcutáneo abdominal.",cycle:"3–6 meses, pausas 4 sem",timing:"Antes de dormir + mañana en ayunas"},
  {name:"Semaglutida",full:"GLP-1 Receptor Agonist",cat:"Pérdida de grasa",color:C.gold,dose:"0.25mg/sem → hasta 2.4mg",benefits:"Control del apetito, pérdida de peso 12–15%, mejora insulina.",recon:"3mg + 1.5ml agua bact. → 2mg/ml\n• 0.25mg = 12.5U · 1mg = 50U",apply:"Subcutáneo: abdomen, muslo o brazo. Rotar.",cycle:"Largo plazo. Reducir gradualmente.",timing:"Mismo día cada semana"},
  {name:"AOD-9604",full:"HGH Fragment Anti-Obesity",cat:"Pérdida de grasa",color:C.scarlet,dose:"300–500 mcg/día",benefits:"Activa lipólisis sin elevar IGF-1. Grasa localizada.",recon:"2mg + 1ml agua bact. → 2000mcg/ml\n• 500mcg = 25U",apply:"Subcutáneo, zona adiposa.",cycle:"4–8 semanas",timing:"Mañana en ayunas, esperar 30 min"},
];
  ...
  if(sel) return (
    <div style={{paddingBottom:110}}>
      ...
      <div style={{padding:"16px 16px 0"}}>
        ...
        <div style={{background:"#FEF9C3",border:"1px solid #FDE047",borderRadius:14,padding:"12px 16px",fontSize:12,color:"#713F12"}}>
          ⚠️ Información educativa. Consulta siempre con el Dr. Rogelio antes de cualquier uso.
        </div>
      </div>
    </div>
  );

  const cats=[...new Set(PEPTIDOS.map(p=>p.cat))];
  return (
    <div style={{paddingBottom:110}}>
      <Hdr title="Péptidos" sub="Protocolos de investigación" imgId="1576091160550-2173dba999ef"/>
      <div style={{padding:"16px 16px 0"}}>
        <div style={{background:"#FEF9C3",border:"1px solid #FDE047",borderRadius:14,padding:"10px 14px",marginBottom:14,fontSize:12,color:"#713F12"}}>
          ⚠️ Información educativa. Consulta con el Dr. Rogelio antes de cualquier uso.
        </div>
        {cats.map(cat=>(
          <div key={cat}>
            <div style={{fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:10,marginTop:8}}>{cat}</div>
            {PEPTIDOS.filter(p=>p.cat===cat).map(p=>(
              <button key={p.name} onClick={()=>setSel(p)} style={{display:"flex",alignItems:"center",gap:14,width:"100%",padding:"16px 18px",background:C.card,border:"none",borderRadius:18,marginBottom:10,boxShadow:C.shadow,textAlign:"left"}}>
                <div style={{width:48,height:48,borderRadius:12,background:p.color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <div style={{width:18,height:18,borderRadius:"50%",border:"2px solid rgba(255,255,255,0.65)"}}/>
                </div>
